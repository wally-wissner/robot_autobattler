...
from backend.asset_loaders import badges, simple_cards
...
def random_rarity(
    min_rarity: ERarity = ERarity.COMMON, max_rarity: ERarity = ERarity.RARE
) -> ERarity:
    defaults = {
...
def generate_badge(
    ...
    options = [badge for badge in badges if badge.rarity == rarity and badge.bp == bp]
...
def generate_card(
    ...
    rarity = random_rarity(*rarity_range)
    bp = random_bp(*bp_range)
    options = [card for card in simple_cards if card.rarity == rarity and card.bp == bp]
...
def generate_team(
    is_player: bool, total_level: int, quality: float, n_units: int = None
) -> Team:
    if not 0 <= quality <= 1:
        raise ValueError(f"Expected quality to be between 0 and 1. Received {quality}.")
