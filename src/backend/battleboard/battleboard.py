...
class BattleBoard(object):
    ...
    def visible_tiles(self, tile: Tile, radius: int, method):
        ...
        if method == "line_of_sight":
            return {
                other
                for other in self.tiling.disk(tile, radius)
                if self.visible_from(tile, other)
            }
    ...
    def generate(self, *args, **kwargs) -> None:
        ...
        self._battle_tiles = {
            tile: BattleTile(position=tile, terrain=self.generator[tile])
    ...
    def units(self, tiles: Tile | Iterable[Tile]) -> Unit | Iterable[Unit]:
        if isinstance(tiles, Tile):
            ...
            raise TypeError(
                "Argument `tiles` must be of type Tile or be an iterable of type Tile."
            )

    def battle_tiles(
        self, tiles: Tile | Iterable[Tile]
    ) -> BattleTile | Iterable[BattleTile]:
        if isinstance(tiles, Tile):
